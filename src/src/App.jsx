...
const PHASE_COLOR = {
  BULLRUN: "#22c55e", BEARMARKET: "#ef4444",
  SIDEWAYS: "#f59e0b", RECOVERY: "#3b82f6",
  KAPITULATION: "#f97316", NEUTRAL: "#6b7280",
};

const fmt = (pair, price) => {
  if (!price) return "—";
  const p = (pair || "").replace("USDT", "");
  if (p === "XRP") return Number(price).toFixed(4);
  if (p === "ETH") return Number(price).toFixed(1);
...
export default function App() {
  ...
  const avgRR = signals.length > 0
    ? (signals.reduce((a, s) => a + (s.rr || 0), 0) / signals.length).toFixed(1)
    : "—";
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f9fafb", fontFamily: "'DM Sans', sans-serif", color: "#111" }}>
      ...
      <div style={{ background: "#fff", borderBottom: "1px solid #e5e7eb", padding: "0 24px", height: "56px", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 10 }}>
        ...
        <button onClick={() => setIsPro(!isPro)} style={{ padding: "6px 14px", background: isPro ? "#111" : "#fff", color: isPro ? "#fff" : "#111", border: "1px solid #e5e7eb", borderRadius: "7px", fontSize: "13px", fontWeight: "500", cursor: "pointer" }}>
          {isPro ? "Pro aktiv" : "Upgrade →"}
        </button>
      </div>

      <div style={{ maxWidth: "660px", margin: "0 auto", padding: "28px 20px" }}>
        {error && (
          <div style={{ background: "#fef2f2", border: "1px solid #fecaca", borderRadius: "8px", padding: "12px 16px", fontSize: "13px", color: "#dc2626", marginBottom: "20px", display: "flex", justifyContent: "space-between" }}>
            <span>⚠ {error}</span>
            <button onClick={fetchSignals} style={{ fontSize: "12px", color: "#dc2626", background: "none", border: "1px solid #fecaca", borderRadius: "5px", padding: "3px 10px", cursor: "pointer" }}>Retry</button>
          </div>
        )}

        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: "10px", marginBottom: "28px" }}>
          {[
            { label: "Win Rate", value: total > 0 ? `${wr}%` : "—", color: "#22c55e" },
            { label: "Signals", value: signals.length || "—", color: "#111" },
            { label: "Ø R:R", value: signals.length > 0 ? `1:${avgRR}` : "—", color: "#f59e0b" },
            { label: "Top Pair", value: "XRP", color: "#3b82f6" },
          ].map(s => (
        ...
        <div style={{ display: "flex", gap: "2px", background: "#f3f4f6", borderRadius: "8px", padding: "3px", marginBottom: "18px" }}>
          {["signals", "performance"].map(t => (
            <button key={t} onClick={() => setTab(t)} style={{ flex: 1, padding: "7px", background: tab === t ? "#fff" : "transparent", border: "none", borderRadius: "6px", fontSize: "13px", fontWeight: tab === t ? "500" : "400", color: tab === t ? "#111" : "#6b7280", cursor: "pointer", boxShadow: tab === t ? "0 1px 3px rgba(0,0,0,0.07)" : "none" }}>
        ...
        {!loading && tab === "signals" && (
          <div>
            {signals.length === 0 && !error && <div style={{ textAlign: "center", padding: "40px", color: "#9ca3af", fontSize: "13px" }}>Noch keine Signals — APEX wartet auf einen Trade.</div>}
            ...
            {signals.map((s, i) => {
              ...
              return (
                <div key={i} style={{ position: "relative", background: "#fff", border: "1px solid #e5e7eb", borderRadius: "10px", padding: "15px 18px", marginBottom: "7px", overflow: "hidden" }}>
                  {locked && (
                    <div style={{ position: "absolute", inset: 0, backdropFilter: "blur(5px)", background: "rgba(249,250,251,0.75)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 2, borderRadius: "10px" }}>
                      <button onClick={() => setIsPro(true)} style={{ padding: "7px 16px", background: "#111", color: "#fff", border: "none", borderRadius: "7px", fontSize: "13px", fontWeight: "500", cursor: "pointer" }}>Freischalten →</button>
                    </div>
                  )}
        ...
        {!loading && tab === "performance" && (
          <div>
            {["BTC", "ETH", "XRP"].map(pair => {
              const ps = signals.filter(s => getPair(s) === pair);
              const pw = ps.filter(s => s.result === "WIN").length;
              const pwr = ps.length > 0 ? Math.round((pw / ps.length) * 100) : 0;
              ...
              return (
                <div key={pair} style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: "10px", padding: "16px 20px", marginBottom: "8px" }}>
                  ...
                  <div style={{ background: "#f3f4f6", borderRadius: "4px", height: "4px", overflow: "hidden" }}>
                    <div style={{ width: `${pwr}%`, height: "100%", background: c, borderRadius: "4px" }} />
                  </div>
